...
def Optim(x):
    try:
      x=x[0]
      fname = "/path/to/storage/"+str(x[0])+" "+str(x[1])+" "+str(x[2])+" "+str(x[3])+" "+str(x[4])+" "+str(x[5])+" "+str(x[6])
      ...
      if x[1] == "Solar-HT-ATES":
          ...
          ATES = ATES_obj([solar],max_V = x[4],HP=None,thickness=50,CO2_elec=200,kh=10,ani=5,T_ground=15,N_wells=6,depth=130,lifetime=30)
          ATES.fix_opex = ATES.fix_opex*x[6]
          ATES.var_opex = ATES.var_opex*x[6]
          ATES.capex = ATES.capex*x[6]
      ...
      df = pd.DataFrame(data={"Size_Geo": [x[3]],"Size_solar":[x[2]],"Size_ATES":[x[4]],"Config":[x[1]],"Demand":[x[0]],"Ratio": [x[5]],"Ratio_ATES":[x[6]],"System_LCOH": [LCOE_Yang],"RES":[RES_share],"HT-ATES efficiency":[ATES.Reff]})    
      save_file = "/path/to/storage/"
      df.to_parquet(save_file+str(x[0])+" "+str(x[1])+" "+str(x[2])+" "+str(x[3])+" "+str(x[4])+" "+str(x[5])+" "+str(x[6]))
...
ratio_ATES = [0.5,1,2]
x_df=pd.DataFrame()
x = pd.DataFrame(0,index = [0], columns = ["demand","config","solar size","geo size","HT-ATES size","ratio","HT-ATES ratio"])
x_list=[]
