...
def system(demand, supply, len_timestep = 3600, time_horizon=8760,control = None):
    ...
    # why not changed for stable supply?
    ...
    if Storage:
        ...
        # Initialize (flow available for demand)
        flow_av_demand = 0
        ...
        for j in storage_obj.supplier:
            if j.name != 'Geothermal well':
                ...
                while k<3:
                    #Set up flow
                    dummy_flow = df_flow["Total flow to storage"]*(loss_percentage) #Does this not always reduce the flow? dummy flowd
                    ...
                    if storage_obj.HP != None:
                        ...
                        storage_obj.init_cold_well(demand.T_out+T_ineff_due_HX,volume) #P: check what reff is used (8th year avg or is it implemented yearly?)
        ...
        df_flow["Total flow to storage"]=df_flow["Total flow to storage"]*percentage #this s flawed if it is chosen to use multiple suppliers?
